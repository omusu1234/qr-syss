...
def get_client_ip():
    """Get client IP address, handling proxy headers and Vercel"""
...
def submit_attendance():
    ...
    try:
        ...
        app.logger.warning(f"Error getting client IP: {str(e)}")
        client_ip = 'unknown'
    
    # Device-based rate limiting: Check if this IP/device has already submitted for this session
    # This prevents multiple people from using the same device for the same session
    # Only check if we have a valid IP (not 'unknown')
    if client_ip and client_ip != 'unknown':
        try:
            ...
            # Log the error but don't block submission if IP check fails
            app.logger.error(f"Error checking device submission: {str(e)}", exc_info=True)
            # Continue with submission if IP check fails (fail open for availability)
            # This ensures the system remains available even if IP checking has issues
